 ...
 * Algo normal pueden ser entre 100 bytes y 1 kb
 ```c
 {
    /// id de este Graphic, puede utilizarse como índice de SAT
    uint8_t id;
    /// dirección base x
    uint8_t xAddress;
    /// dirección base y
...
typedef struct _sGraphic{
    ...
    /// Objeto que permite saltos verticales en el sprite
    sJumpV *sJ;
    /// Función de definiciones del propio objeto gráfico (viene definida en su archivo)
    void (*_def)(struct _sGraphic* sG, const uint8_t id, const uint8_t xAddress, const uint8_t yAddress, const uint16_t prev_tiles, const uint8_t x, const uint8_t y, oMove *oM, sJumpV *sJ);
} sGraphic;


// strcpy(miLibro.titulo, "Programacion en C");
/** Representa un gráfico (fondo, img, sprite o animación) según su archivo de definiciones (ej. "ball_def.h") */
// struct _sGraphic sBall = {
//     _ball_pal, _ball_til, _ball_size, _ball_ntiles, _ball_img, _ball_img_w, _ball_img_h, 0, _ball_ani
// };
