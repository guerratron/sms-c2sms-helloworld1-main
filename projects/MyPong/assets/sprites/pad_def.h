...
#ifndef  __PAD_DEF_H__
#define  __PAD_DEF_H__
...
//#define _pad_pal_size 4
//#define _pad_pal_size _pal_base_size
/// @brief Paleta de trabajo (hasta 16 colores). Los valores son los índices de la paleta global SMS
/*const uint8_t _pad_pal[_pad_pal_size] = {
    0x00, 0x0b, 0x1f, 0x3f
};*/
...
//uint8_t _pad_pal_clon[_pad_pal_size];
...
#define _pad_img_w 1
/// @brief Alto DEL SPRITE MULTI-TILE, UNA IMÁGEN DE SPRITE COMPLETA DE 3 TILE de alto
#define _pad_img_h 3
/// @brief TAMAÑO DEL SPRITE MULTI-TILE, UNA IMÁGEN DE SPRITE COMPLETA DE 3 TILES (w * h)
#define _pad_img_size _pad_img_w * _pad_img_h
/// @brief SIRVE PARA CREAR SPRITES MULTI-TILES, UNA IMÁGEN DE SPRITE COMPLETA DE 3 TILES (1 x 3 tiles).  
/// Los índices se refieren a la posición de cada tile en el 'tileset'
const uint8_t _pad_img[_pad_img_size] = {
    0x00,
    0x01,
    0x02
};
...
/// A través de pruebas he observado que la mejor manera (o quizás la única, si se quieren editar tiles) es 
/// tener un array "const" en la ROM y hacer una copia "volátil" (NO-CONST) en la RAM
uint8_t _pad_img_clon[_pad_img_size];

// ANIMATION
// MODIFICAR EL TAMAÑO ACORDE A LOS DATOS DEL ARRAY '*_ani'

/// @brief TAMAÑO DEL SPRITE ANIMADO (para el array "_pad_ani"), UNA ANIMACIÓN DE SPRITE DE SECUENCIAS DE 3 TILES
#define _pad_ani_size 1
/// @brief PARA CREAR SPRITES CON ANIMACIONES, UNA BOLA ANIMADA DE 3 TILES.  Es un array de índices de tiles del sprite.
const uint8_t _pad_ani[_pad_ani_size] = {
    0x00
};
/// RAM.
/// copia el array original de la ROM para permitir modificación en el puntero (básicamente de ROM a RAM)
/// A través de pruebas he observado que la mejor manera (o quizás la única, si se quieren editar tiles) es
/// tener un array "const" en la ROM y hacer una copia "volátil" (NO-CONST) en la RAM
uint8_t _pad_ani_clon[_pad_ani_size];

/// JUMP (VERTICAL)
// bool
#define _pad_is_jumping false
// int16_t
#define _pad_jump_velocity 4
// Nivel del suelo [uint8_t]
#define _pad_jump_ground_y 120
// Fuerza de gravedad (ajustable) [int16_t]
#define _pad_jump_gravity 1

// MAIN: BITPLANES  
// MODIFICAR SOLO EL NÚMERO DE TILES ACORDE A LOS DATOS DEL ARRAY '*_til'  

/// @brief Número de tiles de los que se compone el array de tiles "_pad_til"
#define _pad_ntiles 3
/// @brief Tamaño en bytes de todos los tiles que se compone el array de tiles "_pad_til"
#define _pad_size _pad_ntiles * 32 // 96
/** BitPlanes:
 * - TileSet, Patterns, Charset (bitplanes). Unique-Tiles
 * - UNIQUE TILES:: [tiles 3 * 32 bytes]
 */
const uint8_t _pad_til[_pad_size] = {
    // Tile 0
    /*0x1f, 0x00, 0x00, 0x00,
    0x1f, 0x00, 0x00, 0x00,
    0x1f, 0x00, 0x00, 0x00,
...
void _padDef(sGraphic *sG, const uint8_t id, const uint8_t xAddress, const uint8_t yAddress, const uint16_t prev_tiles, const uint8_t x, const uint8_t y, oMove *oM, sJumpV *sJ)
{
    // ROM->RAM
    ...
    // pal
    // uint8_t _pal_clon[_pad_pal_size];
    //memcpy(_pad_pal_clon, _pad_pal, _pad_pal_size);
    //memcpy(_pad_pal_clon, _pal_base, _pad_pal_size);
    _palBaseDef(sG); // establece "sG->pal"
    // img
    memcpy(_pad_img_clon, _pad_img, _pad_img_size);
    // ani
    memcpy(_pad_ani_clon, _pad_ani, _pad_ani_size);
    sG->id = id;
    ...
    sG->x = x;
    sG->y = y;
    //sG->pal = _pad_pal_clon;
    sG->tiles = _pad_til;
    sG->size = _pad_size;
    sG->nTiles = _pad_ntiles;
    sG->img = _pad_img_clon;
    sG->w = _pad_img_w;
    sG->h = _pad_img_h;
    sG->i = 0;
    sG->ani_size = _pad_ani_size;
    sG->ani = _pad_ani_clon;
    sG->oM = oM;
    sG->sJ = sJ;
    
    // AJUSTES
    if(sG->sJ){
        sG->sJ->is_jumping = _pad_is_jumping;
        sG->sJ->jump_velocity = _pad_jump_velocity;      // 4;
        sG->sJ->ground_y = _pad_jump_ground_y;           // 120; // Nivel del suelo
        sG->sJ->gravity = _pad_jump_gravity;             // 1;    // Fuerza de gravedad (ajustable)
    }
    // corrección desplazamiento índices de tilesets en img y ani
    uint8_t i = 0;
    // img
    for (i = 0; i < (sG->w * sG->h); i++){  //_pad_img_size
        sG->img[i] += sG->prevTiles;
    }
    // ani
    for (i = 0; i < sG->ani_size; i++){     //_pad_ani_size
