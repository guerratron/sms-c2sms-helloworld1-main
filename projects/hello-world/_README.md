...
>   Esto es un proyecto base LIMPIO para "SMS" (Master System) desde "C" (o "asm"), lo compila 
...
Para más detalle observar "miniheart_def.h" y utilizar como plantilla para nuevos gráficos.  
...
En el código principal utilizar las funciones "init()", en "update()" y "draw()".
...
> el proyecto contendrá el código mínimo LIMPIO sobre el que construir el projecto siguiendo algún tutorial.  
> Si se prueba en un emulador de momento sólo se muestra un fondo como pantalla de inicio y un leve pitido inicial.
...
## DOCUMENTACIÓN
Puede generarse la documentación del proyecto utilizando "Doxygen". Viene con un archivo de configuración **Doxyfile** preparado para generar la documentación en el directorio "/docs" símplemente ejecutar en la consola: ``` > doxygen ```  

Si se quiere personalizar la documentación habría que modificar algunas definiciones de este archivo de configuración como: nombre, descripción, logos, idioma, ..
...
Si se quiere generar documentación hay que tener instalado ``` Doxygen ```.  
