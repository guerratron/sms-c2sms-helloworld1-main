 ...
 * @brief Archivo de definiciones para la creación de un logo de 1 tile (corazoncito).
...
// velocidad de salto [int16_t]
#define _miniheart_jump_velocity 4
...
sGraphic *_miniheartDef(sGraphic *sG, const uint8_t id, const uint8_t xAddress, const uint8_t yAddress, const uint16_t prev_tiles, const uint8_t x, const uint8_t y, oMove *oM, sJumpV *sJ)
{
    ...
    if (sG->sJ){
        ...
        sG->sJ->jump_velocity = _miniheart_jump_velocity;       // 4;
