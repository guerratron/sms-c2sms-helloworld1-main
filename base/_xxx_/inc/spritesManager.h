/**
 * @file spritesManager.h
 * @brief Funciones para dibujado de elementos gráficos.
 * @details Alberga las funciones de carga en memoria y dibujado de los sprites sobre el VDP utilizando las "macros de SDCC". Ver la documentación de "Sprites Definitions".
 * @author GuerraTron26
 * @date 2026
 * @version 1.0
 */
 ...
 * IMPORTANTE !  
 * 
 * La mayoría de funciones comprueban la propiedad "dirty" del objeto gráfico para determinar si el sprite necesita "redibujarse", porque 
 * de no ser así, podemos ahorrarnos el cómputo y el acceso a la memoria del VDP, lo cual nos ahorra ciclos. 
 * De la misma forma al terminar esas funciones se establece esta propiedad en "false" para especificar que no necesita "redibujarse". 
 * 
 * Ha través de varias pruebas se ha comprobado que esta propiedad es la diferencia entre una visualización fluida o no (ya que ahorra 
 * muchos redibujados innecesarios).  
 * Recordar establecer esta propiedad en true antes de llamar a las funciones para poder activarla.
*/

/** Carga la paleta para sprites (la segunda paleta).  
 * Necesita el array de índices de paleta, utiliza internamente "write_palette(..)"  */
...
/** Carga en memoria el array de tiles que representará un sprite (también su paleta), para ello necesita el 
 * objeto gráfico y la dirección donde van los tiles.  
 * Posteriormente habría que utilizar la función toTilesgifyDefineUpdate(), o sus versiones de sprite toAniDefine(), 
 * toAniIndex y toAniPos(), para representarlos en pantalla en el lugar correcto. */
 ...
 * Posteriormente se podrían actualizar esas coordenadas a través de "spr->w y spr->h" sin tener que
 * volver a definir los índices.  
 * Hay que especificar si es el último sprite a representar o no.  
 * Antes de esta función se supone que se habrán cargado los tiles a la zona de sprites en el VDP por ejemplo
 * con draw_sprite(..) o con load_sprite(..);
 * Ej:
 ```c
    #include <ball_def.h>
    load_sprite((sGraphic *) &sBall, (uint16_t) vram_addr);
    toTilesgifyDefineUpdate((sGraphic *) &sBall, -1, -1, -1, false);
 ```
 * La función se encarga también de comprobar si el sprite necesita "redibujarse" a través de su propiedad "dirty". 
 * De la misma forma al terminar la función se establece esta propiedad en "false" para especificar que no necesita "redibujarse".  
 * Recordar establecer esta propiedad en true antes de llamarla para poder activar la función.
 */
void toTilesgifyDefineUpdate(sGraphic *spr, int8_t id, int8_t posX, int8_t posY, uint8_t last){
    if(!spr || !spr->dirty){ return; } // si no es redibujable sale
    if(id > -1){ spr->id = id; }
 ...
 * Hay que especificar si es el último a representar o no  
 * Está más bien preparado para el "init()"
 */
 ...
 * Igual que "toAniDefine(..)" pero no hay que especificar si es el último o no.  
 * Está más bien preparado para el "update() o el draw()"
 */
 ...
 * contador puede utilizarse externamente como se quiera, por ejemplo a modo de "contador de golpeos".
...
int8_t moveWith(sGraphic* sG, bool autoChange){
    ...
    sG->dirty = true; // lo marca como redibujable porque ha cambiado de posición
    return limit;
}
...
/** Pues eso, comprueba si existe colisión entre dos objetos gráficos. */
 ...
 * En cada actualización se modifica la posición "y" y la velocidad de salto.
 */
 ...
 * Hay que volver a especificar la velocidad deseada de salto vertical porque durante la actualización se modifica.
*/
void trigger_jumpV(sGraphic *sG, int16_t vel){
    if (sG->sJ && !sG->sJ->is_jumping) {
        sG->sJ->is_jumping = true;
        sG->sJ->jump_velocity = vel; //8; // Velocidad inicial del salto (impulso hacia arriba)
    }
}
